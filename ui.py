...
def setup_sidebar(tickers: List[str]) -> Dict[str, str]:
    ...
    ticker_option = st.sidebar.selectbox("Select Company/Ticker", tickers)
    res["ticker"] = ticker_option
...
def plot_financial_data(financials: CompanyFinancials, ticker: str) -> None:
    # disclaimer
    st.markdown(
        f"_**Note**: All financial data is extracted from 10-K filings derived \
        from the SEC EDGAR database using a LLM ({MODEL}). Data points may \
        not be 100% accurate, and missing data points indicate that the LLM \
        was unable to retrieve the relevant information from the filing. \
        Refer to the filings for further information and/or clarification._"
    )
